...
_EL_MEAN = (75.84982, 83.605354, 119.32657)
_ER_MEAN = (69.43988, 76.92846, 112.67799)
_F_MEAN = (81.104645, 90.2605, 129.16145)
...
def mean_image_subtraction(image):
  ...
  if image[0].shape[2] != 3:
    raise ValueError('Input must be of size [height, width, C=3]')
  num_channels = image[0].shape[-1]

  el_channels = tf.split(axis=2, num_or_size_splits=num_channels, value=image[0])
  er_channels = tf.split(axis=2, num_or_size_splits=num_channels, value=image[1])
  f_channels = tf.split(axis=2, num_or_size_splits=num_channels, value=image[2])

  for i in range(num_channels):
    el_channels[i] -= _EL_MEAN[i]
    er_channels[i] -= _ER_MEAN[i]
    f_channels[i] -= _F_MEAN[i]

  eye_left = tf.concat(axis=2, values=el_channels)
  eye_right = tf.concat(axis=2, values=er_channels)
  face = tf.concat(axis=2, values=f_channels)
  # return tf.concat(axis=2, values=channels)
  return (eye_left,eye_right, face, image[3])
